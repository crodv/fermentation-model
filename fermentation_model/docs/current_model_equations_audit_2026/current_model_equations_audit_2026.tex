...
\begin{titlepage}
  ...
  \begin{minipage}{0.82\textwidth}
    \small
    \textbf{Alcance.} Auditoría original del 9 de septiembre, actualizada el
    22 de septiembre de 2026 para incorporar la corrección full-theta, la
    conversión SCCM corregida y los estados A/B/C. Se conservan las 75
    relaciones matemáticas y el contraste histórico de 36 entradas. La
    actualización fue de solo lectura sobre código y artefactos versionados;
    no ejecutó optimizadores ni alteró parámetros o resultados científicos.
  \end{minipage}

  \vfill
  {\sffamily\color{auditslate}Auditoría original: 9 de septiembre de 2026\\
   Actualización de vigencia: 22 de septiembre de 2026}\par
  \vspace{0.4cm}
\end{titlepage}
...
El estado actualizado de los tres conflictos originales es:
\begin{enumerate}[label=\textbf{C\arabic*.}]
\item \E{RESUELTO EN B/C; HISTÓRICO EN A}. Los seis parámetros fijos omitidos
de \path{theta.csv} están serializados en \path{theta_natural_full.csv} y
\path{theta_synthetic_full.csv}; B/C exigen 17/17 y no usan fallback. A y los
notebooks antiguos conservan \code{DEFAULT\_THETA} para reproducibilidad.
\item \E{CONFLICTO ABIERTO}. Upstream usó $\Delta N=0.08$ kg m$^{-3}$; CO$_2$
sobrescribe $\Delta N=0.14$ kg m$^{-3}$ y cambia los tiempos.
\item \E{RESUELTO EN SCCM\_CORRECTED}. Los análisis nuevos usan 24.16
L mol$^{-1}$, factor 0.74 y 0.04043919 g L$^{-1}$ h$^{-1}$ SCCM$^{-1}$
a 2 L. La conversión 22.414 queda solo en LEGACY.
\end{enumerate}

El endpoint reproducible con parametrización completa es
\path{run_co2_matrix_cross_validation_2026_full_theta.py}. Su estado C es un
refit computacional completo, pero los límites activos y la mala condición
local impiden interpretar todos sus parámetros naturales como magnitudes
físicas resueltas.
...
co2_solubility_o2_cross_matrix_2026_full_theta.executed.ipynb
 ...
 -> valida theta natural/sintetico/conjunto [17/17; sin fallback]
 -> A: reconstruccion historica; B: full theta sin refit
 -> C: full theta + refit de nueve parametros CO2
 -> observaciones SCCM_CORRECTED y three_state_*.csv
...
Conversión LEGACY histórica:
\begin{equation}
y_{g/L/h}=y_{sccm}\frac{60}{1000}\frac{44.01}{22.414}\frac1{V_L};
\quad V_L=2:\ 1\mathrm{\ SCCM}=0.05890515\mathrm{\ g\,L^{-1}h^{-1}}. \tag{R63}
\end{equation}
\sourceinfo{\E{HISTÓRICO} \code{LEGACY\_SCCM\_CONVERSION}; se conserva para reproducir A LEGACY.}

Conversión SCCM\_CORRECTED vigente para análisis nuevos:
\begin{equation}
y_{g/L/h}=y_{sccm}\frac{44.0095(60)(0.74)}{1000(24.16)(2.0)}
=0.04043919y_{sccm}. \tag{R64}
\end{equation}
\sourceinfo{\E{EXECUTED}\E{CANÓNICO} Validada por el runner full-theta; observaciones y parámetros CO$_2$ se ajustan sobre la misma conversión.}
...
\toprule
\tablehead{Símbolo} & \tablehead{Unidad} & \tablehead{Valor full} & \tablehead{Estado} & \tablehead{Fuente/ecuación} \\
\midrule
\endhead
\midrule
...
\bottomrule
\endlastfoot
$\mu_0$ & h$^{-1}$ & 0.0787279168 & REESTIMATED\_UPSTREAM & \path{theta_natural_full.csv}; R09,R19 \\
$s_N$ & (kg m$^{-3}$)$^{-1}$h$^{-1}$ & 8.75144587 & FIXED\_UPSTREAM & full theta; R09 \\
$q_N$ & h$^{-1}$ & 0.0210803824 & REESTIMATED\_UPSTREAM & full theta; R28 \\
$q_{XG}$ & h$^{-1}$ & 0.0814909864 & FIXED\_UPSTREAM & full theta; R29 \\
$q_{XF}$ & h$^{-1}$ & 0.0709327065 & FIXED\_UPSTREAM & full theta; R30 \\
$\beta_{G0}$ & h$^{-1}$ & 0.4288780030 & REESTIMATED\_UPSTREAM & full theta; R10,R20 \\
$s_G$ & (g L$^{-1}$)$^{-1}$h$^{-1}$ & 0.0974550987 & FIXED\_UPSTREAM & full theta; R10 \\
$\beta_{F0}$ & h$^{-1}$ & 3.4403991748 & REESTIMATED\_UPSTREAM & full theta; R11,R21 \\
$s_F$ & (g L$^{-1}$)$^{-1}$h$^{-1}$ & 0.1782952055 & FIXED\_UPSTREAM & full theta; R11 \\
$q_{EG}$ & h$^{-1}$ & 1.7128067938 & REESTIMATED\_UPSTREAM & full theta; R29 \\
$q_{EF}$ & h$^{-1}$ & 5.0230167502 & REESTIMATED\_UPSTREAM & full theta; R30 \\
$i_G$ & L g$^{-1}$ & 0.0560374373 & REESTIMATED\_UPSTREAM & full theta; R12,R18 \\
$i_E$ & L g$^{-1}$ & 0.0401679410 & REESTIMATED\_UPSTREAM & full theta; R13,R17 \\
$K_{d0}$ & h$^{-1}$ & 0.000150249548 & REESTIMATED\_UPSTREAM & full theta; R24 \\
$m_0$ & h$^{-1}$ & 0.0183385794 & FIXED\_UPSTREAM & full theta; R22 \\
$\gamma_{G0}$ & h$^{-1}$ & 0.1155587914 & REESTIMATED\_UPSTREAM & full theta; R32 \\
$\gamma_{F0}$ & h$^{-1}$ & 0.000100001622 & REESTIMATED\_UPSTREAM & full theta; R32 \\
\end{longtable}
\rowcolors{2}{}{}
\endgroup

El vector completo exige exactamente los 17 nombres. La rama A y los notebooks
LAB013--015/LAB016--018 conservan los antiguos fallbacks 18.0, 0.1125, 0.1125,
0.03, 0.03 y 0.01 para $s_N,q_{XG},q_{XF},s_G,s_F,m_0$.
...
\begin{table}[H]
\centering
\caption{Parámetros CO$_2$ naturales A/B y C sobre SCCM corregido.}
...
{\tablehead{A/B}} & {\tablehead{C refit}} & \tablehead{Estado C} \\
\midrule
kCO2\_release\_h & 0.4243820035 & 0.2890476484 & libre \\
CO2sat\_scale & 0.3500401486 & 2.4999999956 & cota superior \\
O2\_qmax\_mg\_gdw\_h & 0.1500000000 & 3.0511396610 & libre \\
O2\_initial\_scale & 0.1954835969 & 1.4856644260 & cota superior \\
pulse\_t\_rise\_h & 64.6592211849 & 64.3046189928 & libre \\
pulse\_activity\_gain & 0.2500000000 & 0.2500000000 & cota inferior \\
chem\_activation\_start\_fraction & 0.7351793215 & 0.7617698235 & libre \\
chem\_activation\_duration\_fraction & 1.0637830936 & 0.8011851848 & libre \\
matrix\_gain & 2.1390109131 & 1.5422832133 & perfilado/libre \\
\bottomrule
\end{tabularx}
\rowcolors{2}{}{}
\end{table}

Fuente: \path{co2_matrix_cross_validation_2026_full_theta_sccm_corrected/three_state_co2_parameters.csv}.
A/B comparten CO$_2$ y aíslan el cambio de theta; C reajusta los nueve parámetros.
WSSE natural A/B/C: 27.952/54.473/29.342. C recupera el ajuste, pero no resuelve
la identificabilidad física de sus parámetros naturales.
...
\footnotesize
\rowcolors{2}{white}{auditpale!45}
\begin{longtable}{p{0.09\textwidth}p{0.22\textwidth}p{0.31\textwidth}p{0.20\textwidth}}
\caption{Correspondencia entre ecuaciones documentadas y código ejecutado.}\\
\toprule
...
C01--02 & MATCH & cero y señal corregida & \path{apply_sensor_zero_correction} \\
...
Conteo del snapshot original de 36 entradas: 23 MATCH, 1 MISMATCH,
...
La actualización full-theta agrega validaciones 17/17 y SCCM\_CORRECTED, pero no
altera artificialmente ese conteo histórico.

\subsection{Ramas numéricas y vigencia}

\begin{itemize}
\item \textbf{A histórico}: subset theta + \code{DEFAULT\_THETA}, con CO$_2$
SCCM-corregido previo; baseline reproducible.
\item \textbf{B full theta sin refit}: vector completo con los mismos parámetros
CO$_2$ de A; aísla el efecto de theta.
\item \textbf{C full theta refit}: vector completo y nueve parámetros CO$_2$
reajustados; endpoint computacional vigente, con reservas de identificabilidad.
\item LAB013--015 y LAB016--018 conservan la reconstrucción parcial histórica y
no son validaciones del endpoint C.
\end{itemize}

El documento compacto \path{modelo_ecuaciones_2026.tex} combina el theta full
con los parámetros CO$_2$ de A/B. Por tanto describe B, no el refit C.

\section{Modelo actual realmente implementado}

La estructura siguiente es común a A/B/C. El endpoint full-theta requiere usar
conjuntamente el vector completo de la matriz, observaciones SCCM\_CORRECTED y
los parámetros CO$_2$ del estado elegido; no deben mezclarse silenciosamente.
...
\item Observación final: R62; R64 es vigente y R63 queda limitada a LEGACY.
...
\item \path{laboratory_2026/run_co2_matrix_cross_validation_2026_full_theta.py}
\item \path{pilot_2025/run_pilot_2025_co2_solubility_integrated_doe.py}
\item \path{shared/run_secondary_joint_campaign_doe.py}
\end{itemize}

Artefactos vigentes: \path{theta_natural_full.csv},
\path{theta_synthetic_full.csv} y el directorio
\path{co2_matrix_cross_validation_2026_full_theta_sccm_corrected/}, en particular
\path{analysis_manifest.json}, \path{theta_validation.csv},
\path{theta_historical_reconstruction_audit.csv},
\path{three_state_objective_summary.csv} y
\path{three_state_co2_parameters.csv}. Los subsets \path{theta.csv}/
\path{theta_by_case.csv} y los resultados previos se conservan como históricos.
